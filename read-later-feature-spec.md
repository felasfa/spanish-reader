...
### View switching — scroll position saved

`showView()` saves and restores `window.scrollY` per view so navigating away and back lands at the same position:
...
## 12. Vocabulary List — Collapsible Card Design

Vocabulary entries use a collapsible card layout instead of a table. Each entry renders as a single row showing the Spanish word and English translation. Tapping expands to reveal the sentence context and source.

### HTML (generated by `renderVocabulary()`)

```html
<div class="vocab-entry [vocab-new] [vocab-expanded]" data-id="1712345678901">
  <div class="vocab-summary">
    <span class="vocab-word">absurdo</span>
    <span class="vocab-sep">→</span>
    <span class="vocab-translation">absurd</span>
    <div class="vocab-entry-actions">
      <svg class="vocab-chevron" ...><!-- › --></svg>
      <button class="vocab-delete-btn" data-id="..." title="Delete"><!-- trash --></button>
    </div>
  </div>
  <div class="vocab-detail">
    <p class="vocab-sentence-es">¿Es absurdo pensar que la ciencia…</p>
    <p class="vocab-sentence-en">Is it absurd to think that science…</p>
    <div class="vocab-source">
      <a href="https://elpais.com/..." target="_blank">elpais.com</a>
      <span class="vocab-date-text">Apr 9, 2026</span>
    </div>
  </div>
</div>
```

The container is `<div id="vocab-tbody">` (no `<table>`) inside `.vocab-table-wrap`.
...
The chevron rotates 90° via CSS when `.vocab-expanded` is present:
```css
.vocab-chevron { transition: transform 0.2s; }
.vocab-entry.vocab-expanded .vocab-chevron { transform: rotate(90deg); }
.vocab-detail { display: none; }
.vocab-entry.vocab-expanded .vocab-detail { display: block; }
```

### Domain display

Uses the same `cleanDomain()` helper as the reading list to strip subdomains:
```javascript
const domain = e.url
  ? (() => { try { return cleanDomain(new URL(e.url).hostname); } catch { return ''; } })()
  : '';
```

---

## 13. Adapting to a Different Stack
